...
export function QuestionEditorCard({
  ...
  return (
    <div className={cn(
      "rounded-surface border bg-card p-4 sm:p-5 transition-shadow",
      dragging ? "shadow-lg border-primary/40" : "border-border/70",
    )}>
      <div className="flex items-start gap-2.5">
        <button
          type="button"
          {...dragHandleProps}
          className="mt-1 h-8 w-8 shrink-0 grid place-items-center rounded-control text-muted-foreground hover:bg-muted cursor-grab active:cursor-grabbing touch-none"
        ...
        <div className="flex-1 min-w-0 space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="t-caption font-mono text-muted-foreground w-6">{index + 1}.</span>
            <Select value={question.kind} onValueChange={(v) => onChangeKind(v as QuestionKind)}>
              <SelectTrigger className="h-8 w-auto gap-1.5 rounded-control t-caption px-2.5 [&>svg]:hidden">
            ...
            <Badge variant="secondary" className="t-caption">{kindMeta.label}</Badge>
            <OriginBadge origin={question.origin} />
            <div className="ml-auto flex items-center gap-3">
              <label className="flex items-center gap-1.5 t-caption text-muted-foreground">
                Required
                <Switch checked={question.required} onCheckedChange={(v) => onChangeField({ required: v })} />
              </label>
              <Button type="button" size="icon" variant="ghost" className="h-8 w-8" onClick={onDuplicate} aria-label="Duplicate question">
                <Copy className="h-3.5 w-3.5" />
              </Button>
              <Button type="button" size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={onDelete} aria-label="Delete question">
          ...
          <div className="grid gap-2 md:grid-cols-2">
            <Textarea
              ...
              rows={2}
              className="rounded-field text-sm"
            />
            <Textarea
