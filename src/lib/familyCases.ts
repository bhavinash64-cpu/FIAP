...
const familySupabase = supabase as any;
...
export async function listFamilyCases(): Promise<FamilyCaseRow[]> {
  await familySupabase.rpc("expire_stale_family_cases");

  const [{ data: cases, error }, { data: surveys }] = await Promise.all([
    familySupabase.from("family_cases").select("*").order("created_at", { ascending: false }),
    familySupabase.from("surveys").select("id, title_en, title_te"),
  ]);
  ...
  if (responseIds.length) {
    const { data } = await familySupabase
...
export async function getFamilyCaseEvents(caseId: string): Promise<FamilyCaseEvent[]> {
  const { data, error } = await familySupabase
...
export async function getFamilyCaseStats(): Promise<FamilyCaseStats> {
  const { data, error } = await familySupabase.rpc("family_case_stats");
  if (error) throw error;
...
export async function updateFamilyCase(
  ...
  const { error } = await familySupabase.from("family_cases").update(next).eq("id", id);
  if (error) throw error;
...
export async function regeneratePin(id: string): Promise<string> {
  for (let attempt = 0; attempt < 6; attempt++) {
    ...
    const { error } = await familySupabase
      .from("family_cases")
...
export async function regenerateLink(id: string): Promise<string> {
  for (let attempt = 0; attempt < 6; attempt++) {
    ...
    const { error } = await familySupabase.from("family_cases").update({ access_token: token }).eq("id", id);
    if (!error) {
...
export async function deleteFamilyCase(id: string): Promise<void> {
  const { error } = await familySupabase.from("family_cases").delete().eq("id", id);
  if (error) throw error;
...
async function recordEvent(caseId: string, event: string, detail: Record<string, unknown>) {
  const { data: auth } = await supabase.auth.getUser();
  await familySupabase.from("family_case_events").insert({
    case_id: caseId,
    event,
